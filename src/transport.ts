 ...
 * Used by both client modules and each microservice bootstrap, so transport
...
export function buildTransport(prefix: string): ClientOptions {
  ...
  switch (kind) {
    ...
    case 'redis':
      ...
      // reconnecting, and `app.listen()` REJECTS → the process exits. With
 ...
 * Eliminates the `as unknown as MicroserviceOptions` cast at every bootstrap
 * site.
 */
